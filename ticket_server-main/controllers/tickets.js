...
const assignToMe = async (req, res, next) => {
  try {
    const tickets = await Ticket.find({
      ...
      //   },
      // },
    }).populate("category comments pickedBy");
...
const ClientsInProgressTickets = async (req, res, next) => {
  try {
    const _tickets = await Ticket.find({ status: "In Progress", createdBy: req.user.id }).populate("category");
    res.status(200).json({ tickets: _tickets });
  } catch (error) {
    next(error);
  }
...
const allReponedTicketsOf_a_Client = async (req, res, next) => {
  try {
    const tickets = await Ticket.find({
      createdBy: req.user.id,
      status: "Reopened",
...
// temp
const detailTicket = async (req, res, next) => {
  try {
...
const resolvedTickets = async (req, res, next) => {
  try {
    ...
    if (_user.role === "admin" || _user.role === "manager") {
      const _tickets = await Ticket.find({ status: "Resolved" }).populate("currentAgent comments category");
      res.status(200).json({ tickets: _tickets });
    } else if (_user.role === "agent") {
      const _tickets = await Ticket.find({ resolvedBy: _user._id, status: "Resolved" });
      res.status(200).json({ tickets: _tickets });
    } else if (_user.role === "client") {
      const _tickets = await Ticket.find({ createdBy: _user._id, status: "Resolved" }).populate("resolvedBy");
      res.status(200).json({ tickets: _tickets });
    }
  } catch (error) {
    next(error);
...
const allEscalatedTickets = async (req, res, next) => {
  try {
    const tickets = await Ticket.find({ escalated: true, currentAgent: null }).populate("category pickedBy");
    res.json({ tickets });
  } catch (error) {
    next(error);
  }
...
// ticket stats
const getResolvedTicketsPerDay = async (req, res, next) => {
  try {
...
const getEscalatedTicketsPerDay = async (req, res, next) => {
  try {
    const escalatedTickets = await Ticket.aggregate([
    ...
    res.json(escalatedTickets);
  } catch (error) {
    next(error);
  }
};

const getCreatedTicketsPerDay = async (req, res, next) => {
  try {
...
const ticketStats = async (req, res, next) => {
  try {
    ...
    const ticketsPerCategory = await Ticket.aggregate([
    ...
    const reopenedCount = await Ticket.countDocuments({ status: "Reopened" });
...
const updateTicketToOpen = async (req, res) => {
  ...
  ticket.status = 'Open';
  await ticket.save();
  res.json({ message: 'Ticket updated to Open' });
};
